...
.NOTES
    ...
    IMPORTANT: This test file uses REAL OpenSSL-generated certificate files because
    EstClient.psm1 calls OpenSSL directly using the call operator (&), which cannot
    be mocked by Pester.
#>
...
'@
    ...
    # Use REAL certificate and key files generated by OpenSSL for mTLS tests
    # EstClient.psm1 calls OpenSSL directly (cannot be mocked), so we need real files
    $script:MockCertPath = "/tmp/test-mtls-cert.pem"
    $script:MockKeyPath = "/tmp/test-mtls-key.pem"
}
...
Describe "Invoke-EstEnrollment" {
    Context "Happy Path - Successful Enrollment" {
        BeforeEach {
            ...
            # CRITICAL FIX: Use Set-Variable with -Scope to properly set responseHeaders in caller's scope
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                ...
                # Set response headers in the caller's scope using Set-Variable
                if ($ResponseHeadersVariable) {
                    Set-Variable -Name $ResponseHeadersVariable -Value @{
        ...
        It "Uses correct EST endpoint URL path" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param($Uri, [string]$ResponseHeadersVariable)

                # Validate endpoint URL
                $Uri | Should -Be "https://pki:9000/.well-known/est/est-provisioner/simpleenroll"

                if ($ResponseHeadersVariable) {
                    Set-Variable -Name $ResponseHeadersVariable -Value @{ 'Content-Type' = 'application/pkcs7-mime' } -Scope 1
                }
                return Get-MockPkcs7Response
            }
        ...
        It "Uses HTTP POST method" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param($Method, [string]$ResponseHeadersVariable)
        ...
        It "Sends CSR as base64-encoded DER (not PEM)" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param($Body, [string]$ResponseHeadersVariable)
        ...
        It "Sets Authorization header with Bearer token format" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param($Headers, [string]$ResponseHeadersVariable)
        ...
        It "Sets Content-Type header to application/pkcs10" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param($ContentType, [string]$ResponseHeadersVariable)
    ...
    Context "Bootstrap Token Redaction" {
        BeforeEach {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param([string]$ResponseHeadersVariable)
                Set-Variable -Name $ResponseHeadersVariable -Value @{ 'Content-Type' = 'application/pkcs7-mime' } -Scope 1
                return Get-MockPkcs7Response
            }
        }
    ...
    Context "Error Handling - 401 Unauthorized" {
        It "Throws exception when bootstrap token is invalid (401)" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                # Simplified HTTP exception pattern matching EstClient's error handling
            ...
            { Invoke-EstEnrollment `
                ...
                -ProvisionerName "est-provisioner" `
                -CsrPem $script:TestCsrPem `
                -BootstrapToken "invalid-token" } | Should -Throw "*401 Unauthorized*Invalid or expired bootstrap token*"
        }
    ...
    Context "Error Handling - 500 Internal Server Error" {
        It "Throws exception when CA has internal error (500)" {
            ...
            { Invoke-EstEnrollment `
                ...
                -ProvisionerName "est-provisioner" `
                -CsrPem $script:TestCsrPem `
                -BootstrapToken "test-token" } | Should -Throw "*500 Internal Server Error*CA internal error*"
        }
    }
    ...
    Context "Error Handling - Invalid Content-Type" {
        ...
        It "Logs error when Content-Type validation fails" {
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param([string]$ResponseHeadersVariable)
                Set-Variable -Name $ResponseHeadersVariable -Value @{ 'Content-Type' = 'application/json' } -Scope 1
                return "{}"
            }
...
Describe "Invoke-EstReenrollment" {
    Context "Happy Path - Successful Re-enrollment" {
        BeforeEach {
            ...
            # Mock successful PKCS#7 response with proper scope handling
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param([string]$ResponseHeadersVariable, $Certificate)
        ...
        It "Passes client certificate for mTLS authentication" {
            ...
            Mock -ModuleName EstClient -CommandName Invoke-RestMethod {
                param($Certificate, [string]$ResponseHeadersVariable)
    ...
    Context "Error Handling - 500 Internal Server Error" {
        ...
        It "Throws exception when CA has internal error (500)" {
            ...
            { Invoke-EstReenrollment `
                ...
                -CsrPem $script:TestCsrPem `
                -ExistingCertPath $script:MockCertPath `
                -ExistingKeyPath $script:MockKeyPath } | Should -Throw "*500 Internal Server Error*CA error*"
        }
    }
