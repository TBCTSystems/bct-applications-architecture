...
## Prerequisites
Before starting, ensure that you have already generated the necessary **self-signed certificates**. These should include:
- A **Certificate Authority (CA) certificate and key**
- A **Vault server certificate and key** (signed by the CA)
- A **Client certificate and key** (signed by the CA)

If you haven’t generated these yet, please follow the **[certificate generation guide](../../docs/miscellaneous/self-signed-certs.md)** before proceeding.
...
In production, you’d use valid certificates (signed by a known CA) and possibly more sophisticated policies. But for a **proof-of-concept**, we’ll focus on how Vault can do **mTLS** (mutual TLS) for authentication and secret management.

## Table of Contents
1. [Introduction](#1-introduction)
2. [Directory Structure](#2-directory-structure)
3. [Configuration and Start Vault](#3-configuration-and-start-vault)
4. [Enable Cert Auth & Create Policy](#4-enable-cert-auth--create-policy)
5. [Minimal .NET 9 Console App](#5-minimal-net-9-console-app)
...
## 3. Configuration and Start Vault
...
### 3.2. Start and Unseal Vault
Follow the instructions [here](../../docs/miscellaneous/unseal.md).
...
### 4.2 Create a New Policy (dotnet-policy)
...
Copy `dotnet-policy.hcl` to docker. From another terminal (outside of the container shell) `cd` to the config folder that contains `dotnet-policy.hcl` and run:
```bash
