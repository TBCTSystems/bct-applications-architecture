...
# NOTE: Logger module is imported by the calling script (acme-agent.ps1 / est-agent.ps1)
# Do not import it here, as $PSScriptRoot is empty when modules are loaded via Import-Module
...
function Invoke-SchemaValidation {
    ...
    # 6. Validate CRL configuration (if present and enabled)
    if ($Config.ContainsKey('crl') -and $Config['crl'] -is [hashtable]) {
        $crlConfig = $Config['crl']

        # If CRL is enabled, validate required CRL fields
        if ($crlConfig.ContainsKey('enabled') -and $crlConfig['enabled'] -eq $true) {

            # Validate CRL URL is a valid URI
            if ($crlConfig.ContainsKey('url')) {
                $crlUrl = $crlConfig['url']
                $uri = $null

                # Check if URL is a non-empty string
                if ($crlUrl -isnot [string] -or [string]::IsNullOrWhiteSpace($crlUrl)) {
                    $errorMsg = "Configuration validation failed: CRL 'url' must be a non-empty string when CRL is enabled"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.url'; value = $crlUrl}
                    throw $errorMsg
                }

                # Validate URI format
                if (-not [System.Uri]::TryCreate($crlUrl, [System.UriKind]::Absolute, [ref]$uri)) {
                    $errorMsg = "Configuration validation failed: CRL 'url' must be a valid absolute URI (got: '$crlUrl')"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.url'; value = $crlUrl}
                    throw $errorMsg
                }

                # Validate URI scheme is http or https
                if ($uri.Scheme -notin @('http', 'https')) {
                    $errorMsg = "Configuration validation failed: CRL 'url' must use http or https scheme (got: '$($uri.Scheme)')"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.url'; value = $crlUrl; scheme = $uri.Scheme}
                    throw $errorMsg
                }

                Write-LogDebug -Message "CRL URL validated successfully" -Context @{url = $crlUrl; scheme = $uri.Scheme}
            } else {
                # CRL is enabled but no URL provided
                $errorMsg = "Configuration validation failed: CRL 'url' is required when CRL is enabled"
                Write-LogError -Message $errorMsg -Context @{field = 'crl.url'; crl_enabled = $true}
                throw $errorMsg
            }

            # Validate CRL cache_path
            if ($crlConfig.ContainsKey('cache_path')) {
                $crlCachePath = $crlConfig['cache_path']

                # Check if cache_path is a non-empty string
                if ($crlCachePath -isnot [string] -or [string]::IsNullOrWhiteSpace($crlCachePath)) {
                    $errorMsg = "Configuration validation failed: CRL 'cache_path' must be a non-empty string"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.cache_path'; value = $crlCachePath}
                    throw $errorMsg
                }

                # Validate path is absolute (starts with / on Linux/Mac, or drive letter on Windows)
                $isAbsolutePath = $false
                if ($crlCachePath.StartsWith('/')) {
                    $isAbsolutePath = $true  # Unix-style absolute path
                } elseif ($crlCachePath -match '^[A-Za-z]:\\') {
                    $isAbsolutePath = $true  # Windows-style absolute path (C:\, D:\, etc.)
                }

                if (-not $isAbsolutePath) {
                    $errorMsg = "Configuration validation failed: CRL 'cache_path' must be an absolute path (got: '$crlCachePath')"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.cache_path'; value = $crlCachePath}
                    throw $errorMsg
                }

                # Validate parent directory exists (or is the root)
                $parentDir = Split-Path -Parent $crlCachePath
                if ($parentDir -and -not (Test-Path $parentDir)) {
                    Write-LogWarning -Message "CRL cache parent directory does not exist (will be created if needed)" -Context @{
                        field = 'crl.cache_path'
                        value = $crlCachePath
                        parent_dir = $parentDir
                    }
                }

                Write-LogDebug -Message "CRL cache_path validated successfully" -Context @{cache_path = $crlCachePath}
            } else {
                # CRL is enabled but no cache_path provided
                $errorMsg = "Configuration validation failed: CRL 'cache_path' is required when CRL is enabled"
                Write-LogError -Message $errorMsg -Context @{field = 'crl.cache_path'; crl_enabled = $true}
                throw $errorMsg
            }

            # Validate max_age_hours if present
            if ($crlConfig.ContainsKey('max_age_hours')) {
                $maxAgeHours = $crlConfig['max_age_hours']

                # Type check (number)
                if ($maxAgeHours -isnot [int] -and $maxAgeHours -isnot [double]) {
                    $errorMsg = "Configuration validation failed: CRL 'max_age_hours' must be a number (got: $($maxAgeHours.GetType().Name))"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.max_age_hours'; value = $maxAgeHours; type = $maxAgeHours.GetType().Name}
                    throw $errorMsg
                }

                # Range check (must be positive, reasonable upper bound 168 hours = 1 week)
                if ($maxAgeHours -lt 0.1 -or $maxAgeHours -gt 168.0) {
                    $errorMsg = "Configuration validation failed: CRL 'max_age_hours' must be between 0.1 and 168.0 (got: $maxAgeHours)"
                    Write-LogError -Message $errorMsg -Context @{field = 'crl.max_age_hours'; value = $maxAgeHours; constraint = '0.1-168.0'}
                    throw $errorMsg
                }
            }

            Write-LogInfo -Message "CRL configuration validated successfully" -Context @{
                enabled = $true
                url = $crlConfig['url']
                cache_path = $crlConfig['cache_path']
            }
        }
    }

    # 7. Check for additional properties (schema has "additionalProperties": false)
