...
public class CertificateManager : ICertificateManager, IDisposable
{
    ...
    public CertificateManager(ILogger<CertificateManager> logger)
    {
        ...
        _logger.LogInformation("Web API Certificate Manager initialized with paths: Cert={CertPath}, Key={KeyPath}, CA={CaPath}", 
    ...
    private async Task<X509Certificate2?> LoadClientCertificateAsync()
    {
        try
        {
            ...
            var chainPath = "/certs/fullchain.pem";
            if (File.Exists(chainPath))
            {
                var chainPem = await File.ReadAllTextAsync(chainPath);
                var certificate = X509Certificate2.CreateFromPem(chainPem, keyPem);
                
                _logger.LogInformation("Loaded Web API client certificate: Subject={Subject}, Expires={Expires}", 
            ...
            {
                // Fallback to individual cert file
                var certificate = X509Certificate2.CreateFromPem(certPem, keyPem);
                
                _logger.LogInformation("Loaded Web API client certificate (fallback): Subject={Subject}, Expires={Expires}", 
                    certificate.Subject, certificate.NotAfter);
        ...
        {
            _logger.LogError(ex, "Failed to load Web API client certificate from {CertPath}", _certPath);
            return null;
        }
    }
    ...
    private async Task<X509Certificate2?> LoadCaCertificateAsync()
    {
        try
        {
            ...
            _logger.LogInformation("Loaded CA certificate for Web API: Subject={Subject}", certificate.Subject);
    ...
    private async void OnCertificateFileChanged(object sender, FileSystemEventArgs e)
    {
        ...
        {
            _logger.LogInformation("Web API certificate file changed, reloading...");
